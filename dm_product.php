...
<html lang="en">
  ...
  <body>
        ...
        <h3><i class="fas fa-boxes mr-2"></i>Select Product</h3>
        <hr>
        <?php 
            if(isset($done)){
        ?>
        <h5 class="text-danger">You already evaluate this product</h5>
            <?php } ?>
        <form action="#" method="post">
                    <div class="form-group">
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                ...
                                ?>
                                <option value='<?php echo $rows['code_product']?>' selected><?php echo $rows['name_product']?></option>
                                    <?php
                                    $result = "SELECT * FROM product ";
                                    $query = mysqli_query($conn,$result);
                                    while($row1=mysqli_fetch_array($query)){
                                    echo "<option value='$row1[code_product]'>$row1[name_product]</option>";
                                    }
                                    ?>
                                </select>
                            </div>
                    ...
                    <?php 
                    if(!$_SESSION['result']){?>
                        <button type="submit" class="btn btn-primary p-2 pr-5 pl-5" style="margin-left: 150px" name="evaluate">Evaluate Now</button><button type="submit" class="btn btn-primary p-2 pr-5 pl-5" style="margin-left: 50px"name="selectproduct">Detail Product</button>
                    <?php }else{?>
                        <button type="submit" class="btn btn-primary p-2 pr-5 pl-5" style="margin-left: 150px" name="evaluate">Evaluate Now</button><button type="submit" class="btn btn-primary p-2 pr-5 pl-5" style="margin-left: 50px"name="selectproduct">Detail Product</button>
                        <button type="submit" class="btn btn-danger p-2 pr-5 pl-5" style="margin-left: 50px"name="resetproduct">Reset Result</button>
                    <?php } ?>
                    </div>         
                    <br>
                    <br>

...
<html lang="en">
  ...
  <body>
        ...
        <h4>
            ...
            ?>
            <h5 class="text-danger"> Product Criteria Already Available </h5>
            <?php } ?>
            <?php
            if(isset($_GET["cannot"])){
            ?>
            <h5 class="text-danger"> Cannot Delete Product Criteria Because Entity Associated With The System </h5>
            <?php } ?>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM product_criteria INNER JOIN criteria ON product_criteria.code_criteria = criteria.code_criteria INNER JOIN product ON product_criteria.code_product = product.code_product";
                ...
                echo "
                    ...
                    <td colspan='2' class='text-center'><a class='m-2' href='admin_addproductcriteria.php?deleteproductcriteria=$row[id]'><i class='fas fa-trash-alt bg-danger text-white p-2 rounded'></i></a></td>
                    </tr>";
                    $no++;
                }
            ?>
            ...
            <form action="#" method="post">
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-md-3">
                                <label for="alternative">Criteria Name</label>
                            </div>
                            <div class="col-md-9">
                                <select name="criteria" id="type">
                                    <option value='' selected>-- Choose Criteria --</option>
                                    <?php
                                    $no = 1;
                                    $result = "SELECT * FROM criteria ";
                                    $query = mysqli_query($conn,$result);
                                    while($row2=mysqli_fetch_array($query)){
                                    echo "<option value='$row2[code_criteria]'>$no. $row2[name_criteria]</option>";
                                    $no++;
                                    }
                                    ?>
                                </select>
                    ...
                    <div class="text-center mb-5">
                        <button type="submit" class="btn btn-primary p-2 pr-5 pl-5" name="addproductcriteria">Add Criteria</button>
                    </div>
                    <br>
                    <br>
                    </form>

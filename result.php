...
<html lang="en">
  ...
  <body>
        ...
        <h3><i class="fas fa-tachometer-alt mr-2"></i>Result</h3>
        <hr>
        <?php 
        $query1 = "SELECT * FROM separation INNER JOIN alternative  ON separation.code_alternative = alternative.code_alternative ORDER BY separation.c ASC";
        $sql1 = mysqli_query($conn, $query1);
        $row1 = mysqli_fetch_array($sql1);
        $product = $_SESSION['product'];
        $query2 = "SELECT * FROM product WHERE code_product = '$product'";
        $sql2 = mysqli_query($conn, $query2);
        $row2 = mysqli_fetch_array($sql2);
        ?>
        <h5>Based on all off decision maker assesment, our calculation by using intuitionistic FUZZY TOPSIS and entropy weighting method. <b><?php echo $row1['name_alternative']?></b> is recommended to supply <b class="bg-info"><?php echo $row2['name_product']?></b> </h5> 
        <table class="table table-striped table-bordered">
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM separation INNER JOIN alternative  ON separation.code_alternative = alternative.code_alternative ORDER BY separation.c ASC";

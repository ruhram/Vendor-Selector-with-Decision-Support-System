...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-cubes mr-2"></i> Criteria</h3>
            <hr>
            <?php
            if(isset($_GET["success"])){
            ?>
            <h5 class="text-danger"> Criteria Has Been Added </h5>
            <?php } ?>
            <?php
            if(isset($_GET["delete"])){
            ?>
            <h5 class="text-danger"> Criteria Has Been Deleted </h5>
            <?php } ?>
            <?php
            if(isset($_GET["edit"])){
            ?>
            <h5 class="text-danger"> Criteria Has Been Edited </h5>
            <?php } ?>
            <?php
            if(isset($_GET["already"])){
            ?>
            <h5 class="text-danger"> Criteria Already Available </h5>
            <?php } ?>
            <?php
            if(isset($_GET["cannot"])){
            ?>
            <h5 class="text-danger"> Cannot Delete Criteria Because Entity Associated With The System </h5>
            <?php } ?>
            <a href="admin_addcriteria.php" class="btn btn-primary mb-3"><i class="fas fa-plus mr-2"></i>Add Criteria</a>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM criteria ORDER BY code_criteria+0";
                ...
                echo "
                    ...
                    <td>$row[type_criteria]</td>
                    <td class='text-center'><a href='admin_detailcriteria.php?code=$row[code_criteria]'><i class='fas fa-info bg-primary p-2 pr-3 pl-3 text-white rounded mr-0' data-toggle='tooltip' title='edit'></i></a></td>
                    <td colspan='2' class='text-center'><a href='edit_criteria.php?code=$row[code_criteria]'><i class='fas fa-edit bg-warning p-2 text-white rounded mr-0' data-toggle='tooltip' title='edit'></i></a>
                    <a class='m-2' href='admin_criteria.php?deletecriteria=$row[code_criteria]'><i class='fas fa-trash-alt bg-danger text-white p-2 rounded'></i></a></td>
                    </tr>";
                    $no++;
                }
            ?>

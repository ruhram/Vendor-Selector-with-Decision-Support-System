...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-user-tie mr-2"></i> Decision Maker</h3>
            <hr>
            <?php
            if(isset($_GET["success"])){
            ?>
            <h5 class="text-danger"> Decision Maker Has Been Added </h5>
            <?php } ?>
            <?php
            if(isset($_GET["delete"])){
            ?>
            <h5 class="text-danger"> Decision Maker Has Been Deleted </h5>
            <?php } ?>
            <?php
            if(isset($_GET["edit"])){
            ?>
            <h5 class="text-danger"> Decision Maker Has Been Edited </h5>
            <?php } ?>
            <?php
            if(isset($_GET["already"])){
            ?>
            <h5 class="text-danger"> Decision Maker Already Available </h5>
            <?php } ?>
            <?php
            if(isset($_GET["cannot"])){
            ?>
            <h5 class="text-danger"> Cannot Delete Decision Maker Because Entity Associated With The System </h5>
            <?php } ?>
            <a href="admin_adddm.php" class="btn btn-primary mb-3"><i class="fas fa-plus mr-2"></i>Add Decision Maker</a>
            ...
            <tbody>
            <?php
                $query = "SELECT * FROM decisionmaker ORDER BY code_dm ASC";
                ...
                echo "
                    ...
                    <td>$row[name_dm]</td>
                    <td>$row[nik_dm]</td>
                    <td>$row[position_dm]</td>
                    <td>$row[importance_dm]</td>
                    <td colspan='2' class='text-center'><a href='edit_dm.php?code=$row[code_dm]'><i class='fas fa-edit bg-warning p-2 text-white rounded mr-0' data-toggle='tooltip' title='edit'></i></a>
                    <a class='m-2' href='admin_decisionmaker.php?deletedm=$row[code_dm]'><i class='fas fa-trash-alt bg-danger text-white p-2 rounded'></i></a></td>
                    </tr>";
                    $no++;
                }
            ?>

...
<html lang="en">
  ...
  <body>
        ...
        <div class="col-md-10 p-5">
            <h3><i class="fas fa-users-cog mr-2"></i> Admin</h3>
            <hr>
            <?php
            if(isset($_GET["success"])){
            ?>
            <h5 class="text-danger"> Admin Has Been Added </h5>
            <?php } ?>
            <?php
            if(isset($_GET["delete"])){
            ?>
            <h5 class="text-danger"> Admin Has Been Deleted </h5>
            <?php } ?>
            <?php
            if(isset($_GET["edit"])){
            ?>
            <h5 class="text-danger"> Admin Has Been Edited </h5>
            <?php } ?>
            <?php
            if(isset($_GET["already"])){
            ?>
            <h5 class="text-danger"> Admin Already Available </h5>
            <?php } ?>
            <?php
            if(isset($_GET["cannot"])){
            ?>
            <h5 class="text-danger"> Cannot Delete Admin Because There Is No Admin In System</h5>
            <?php } ?>v
            <a href="admin_addadmin.php" class="btn btn-primary mb-3"><i class="fas fa-plus mr-2"></i>Add Admin</a>
            ...
            <?php
                ...
                echo "
                    ...
                    <td>$row[nik_admin]</td>
                    <td>$row[posisi_admin]</td>
                    <td>$row[importance_admin]</td>
                    <td>$row[username_admin]</td>
                    <td>$row[password_admin]</td>
                    <td colspan='2' class='text-center'><a href='edit_admin.php?code=$row[id_admin]'><i class='fas fa-edit bg-warning p-2 text-white rounded mr-0' data-toggle='tooltip' title='edit'></i></a>
                    <a class='m-2' href='admin_admin.php?deleteadmin=$row[id_admin]'><i class='fas fa-trash-alt bg-danger text-white p-2 rounded'></i></a></td>
                    </tr>";
                    $no++;
                }
            ?>
